...
class MarginCalculator:
    ...
    def get_cash_margin(self, account: TradingAccount, force_refresh: bool = True) -> float:
        ...
        try:
            ...
            if response.get('status') == 'success':
                funds_data = response.get('data', {})
                # Margin eats from collateral first, then cash
                ...
                cash_margin = available_cash - max(0, collateral - used_margin)
                logger.debug(f"[CASH MARGIN] Cash margin for {account.account_name}: {cash_margin:,.2f} (availablecash={available_cash:,.2f} - collateral={collateral:,.2f} - utiliseddebits={used_margin:,.2f})")
                return cash_margin
            else:
